...
def calculate_dop(point, radars):
    ...
    try:
        ...
        return float('Самопересечение или сингулярная матрица')  
...
class DopCalculatorApp:
    def __init__(self, root):
        ...
        self.save_plot_button = tk.Button(self.top_frame, text="Сохранить график", command=self.export_plot,font=('Arial', 15))
        ...
        tk.Button(self.center2_frame, text="Построить основную точку", command=self.plot_main_point, font=font_left).pack(anchor='w')
        ...
        tk.Button(self.center2_frame, text="Удалить выбранный радар", command=self.remove_selected_radar,font=font_left).pack(anchor='w', pady=5)

        
        tk.Button(self.left_frame, text="Сбросить все значения", command=self.reset, font=font_left).pack(anchor='w', pady=50, )
        tk.Button(self.left_frame, text="Подтвердить значения введенных данных", command=self.confirm,font=font_left).pack(anchor='w', pady=5)

        self.calculate_button = tk.Button(self.left_frame, text="Вычислить DOP", command=self.calculate, font=font_left, state=tk.DISABLED)
    ...
    def update_widget(self, widget, scale_factor):
            ...
            if current_font:
                ...
                if len(font_info) >= 1:
                    try:
                        ...
                        new_font_size = min(int(font_size * scale_factor),30)  
        ...
        if isinstance(widget, tk.Canvas):
            ...
            if isinstance(width, (int, float)) and isinstance(height, (int, float)):
                new_width = min(int(width * scale_factor), 800)  
                new_height = min(int(height * scale_factor), 600)  
    ...
    def plot_main_point(self):
        try:
            ...
            self.ax.plot(point[0], point[1], 'ro', markersize=8, label="Основная точка")  
    ...
    def add_radars(self):
        try:
            ...
            for _ in range(num_radars):
                ...
                error_label = tk.Label(dialog, fg='red')
                ...
                def submit():
                    ...
                    if radar:
                        try:
                            ...
                            error_label.config(text="Введите два числа через пробел.")  
